      ...
      <div className="flex flex-wrap gap-2">
        {people.map((person, index) => (
          <Chip
            ...
            backgroundColor: selectedChips.includes(person) ? '#3B82F6' : '#FFFFFF',
            fontWeight:'bold'}}
      ...
      <div className="border border-blue-500 text-blue-500 rounded-md text-base p-4 w-1/2 mb-4 ml-2">
        ...
        <ul className="pl-0">
          {availabilityData.map((entry, index) => (
            <li key={index} className="text-black flex items-center justify-between text-base mb-2">
...
const GroupAvailabilityComponent = () => {
    ...
    return (
        <div className="w-full lg:w-2/3 bg-white rounded-md shadow-md items-center justify-center py-8 px-8 mx-28 mb-8">
        <h2 className="text-2xl font-bold mb-4 p-3 text-center">Group Availability</h2>
            <div className="flex">
                <People />
                <BestTimesComponent availabilityData={availabilityData} totalPeople={totalPeople} />
            </div>
        </div>
    );
};
